export default {
  async fetch(request, env, ctx) {
    ...
    if (url.pathname === "/api/schema" && request.method === "GET") {
      try {
        const tablesQuery = await env.DB.prepare("PRAGMA table_list;").all();
        ...
        for (const table of tables) {
          const columnsQuery = await env.DB.prepare(`PRAGMA table_info(${table});`).all();
          schema[table] = columnsQuery.results.map(c => c.name);
        }
    ...
    if (url.pathname === "/api/query" && request.method === "POST") {
      try {
        ...
        const { results } = await env.DB.prepare(`SELECT ${safeColumn} FROM ${safeTable}`).all();
    ...
    if (url.pathname === "/api/concerns" && request.method === "GET") {
      try {
        ...
        const { results } = await env.DB.prepare("SELECT * FROM concerns ORDER BY id DESC").all();
        return Response.json({ results });
      } catch (err) {
        return Response.json({ error: err.message }, { status: 500 });
      }
    ...
    // POST: Add a concern manually via the Dev Page
    if (url.pathname === "/api/concerns" && request.method === "POST") {
      try {
        const { text } = await request.json();
        const info = await env.DB.prepare("INSERT INTO concerns (text) VALUES (?)").bind(text).run();
        const insertedId = info.meta.last_row_id;
        
        let aiOutput = "No AI binding configured.";
        if (env.AI) {
          const departments = await getSystemDepartments(env.DB);
          const systemPrompt = buildClassificationPrompt(text, departments, []);
          ...
          aiOutput = sanitizeJsonOutput(aiRes.response || JSON.stringify(aiRes));
          
          await env.DB.prepare("UPDATE concerns SET status = 'processed', ai_output = ? WHERE id = ?")
            .bind(aiOutput, insertedId)
            .run();
        }
    ...
    // POST: Run compilation over all lingering unprocessed entries
    if (url.pathname === "/api/compile-all" && request.method === "POST") {
      try {
        const { results } = await env.DB.prepare("SELECT * FROM concerns WHERE status = 'unprocessed'").all();
        if (results.length === 0) {
          return Response.json({ message: "No unprocessed concerns found." });
        }
        ...
        for (const row of results) {
          let aiOutput = "AI compilation skipped";
          if (env.AI) {
            const systemPrompt = buildClassificationPrompt(row.text, departments, []);
          ...
          await env.DB.prepare("UPDATE concerns SET status = 'processed', ai_output = ? WHERE id = ?")
            .bind(aiOutput, row.id)
            .run();
          
          logs.push(`Processed ID ${row.id}`);
        }
...
// =========================================================================
// HELPER LOGIC FOR AI INFERENCE CONFIGURATION
// =========================================================================

async function getSystemDepartments(db) {
  try {
    const { results } = await db.prepare("SELECT id, name, description FROM Department").all();
    return results;
  } catch (e) {
    return [{ id: 1, name: "General Administration", description: "Default operations endpoint" }];
  }
}
...
function buildClassificationPrompt(concern, departments, existingKeywords = []) {
  return `You are a deterministic classification engine for student concern analysis.

Your job is to convert a raw student concern into structured JSON.

You do NOT give advice.
You do NOT explain reasoning.
You do NOT include extra text.
...
2. Existing Keywords (array of strings)
${JSON.stringify(existingKeywords)}

3. Departments (array of objects)
...
2. Match it to up to 2 most relevant departments.
3. Match as many existing keywords as accurately possible.
4. Only create new keywords if NO existing keyword fits the concept.
...
- Keywords must match EXACT strings from input
- New keywords only if absolutely necessary
- No explanations
- No markdown
- No extra keys
- No trailing commas
...
Be consistent, conservative, and precise.
Prefer existing keywords over creating new ones.
Prefer fewer departments over guessing.

If uncertain, choose the most general department rather than inventing a new interpretation.`;
}
  ...
  <style>
    ...
    * { box-sizing: border-box; margin: 0; padding: 0; font-family: system-ui, sans-serif; }
    ...
    .toggle-btn {
      ...
      border-radius: 0.375rem; cursor: pointer; text-decoration: none; font-weight: 500;
    }
    ...
    select, textarea, input {
      ...
      padding: 0.5rem; border-radius: 0.375rem; font-size: 0.9rem;
    }
    select:focus, textarea:focus { outline: 2px solid var(--accent); }
    button.primary {
      background: var(--accent); color: white; border: none; padding: 0.5rem 1rem; border-radius: 0.375rem; cursor: pointer;
    }
    ...
    .flyout {
      ...
      box-shadow: -4px 0 15px rgba(0,0,0,0.5); transition: right 0.3s ease;
...
<body>
  ...
  <header>
    <a href="${isHome ? '/dev' : '/'}" class="toggle-btn">
      ${isHome ? '➔ Switch to Dev Page' : '➔ Switch to Home Page'}
    </a>
  ...
  <main style="${isHome ? 'grid-template-columns: 1fr;' : 'grid-template-columns: 1fr 1fr;'}">
    ...
    ${isHome ? `
      ...
      <div id="flyoutMenu" class="flyout">
        ...
        <div class="flyout-content" id="unprocessedContainer">
          Loading stack...
        </div>
        <div class="flyout-footer">
          <button class="primary" style="width: 100%; padding: 0.75rem;" onclick="compileAllUnprocessed()">Compile All via AI</button>
        </div>
      </div>
    ` : `
      <section class="card">
  ...
  <script>
    ...
    // =========================================================================
    // HOME VIEW RUNTIME CONTROLLERS
    // =========================================================================
    if (${isHome}) {
      function toggleFlyout(open) {
        document.getElementById('flyoutMenu').classList.toggle('open', open);
      }
      ...
      async function loadSchemaMetaData() {
        ...
        } catch(e) { console.error("Error fetching schema config mapping", e); }
      }
      ...
      function updateColumnDropdown() {
        ...
        if (table && dbSchema[table]) {
          dbSchema[table].forEach(col => {
            colSel.innerHTML += \`<option value="\${col}">\${col}</option>\`;
          });
        }
      }
      ...
      async function runPredefinedQuery() {
        ...
        try {
          const res = await fetch('/api/query', {
            ...
            body: JSON.stringify({ column, table })
          });
          const data = await res.json();
          ...
          tableBody.innerHTML = data.results.map(row => 
            '<tr>' + columns.map(c => {
              const cellVal = row[c];
              return \`<td>\${typeof cellVal === 'object' && cellVal !== null ? JSON.stringify(cellVal) : (cellVal !== null ? cellVal : '')}</td>\`;
            }).join('') + '</tr>'
          ).join('');
      ...
      async function fetchConcernsPipeline() {
        try {
          ...
          data.results.forEach(item => {
            ...
            el.innerHTML = \`
              <div><strong>ID \${item.id}:</strong> \${item.text}</div>
              <span class="status-badge status-\${item.status}">\${item.status}</span>
              \${item.ai_output ? \`<div style="margin-top:0.5rem;"><strong>Analysis Mapping:</strong> \${formatOutputText(item.ai_output)}</div>\` : ''}
            \`;
            allList.appendChild(el.cloneNode(true));

            if(item.status === 'unprocessed') {
              unprocessedCount++;
              unprocList.appendChild(el);
            }
          });

          if(unprocessedCount === 0) {
            unprocList.innerHTML = '<div style="color:var(--text-muted)">All tasks cleared in pipeline stack context.</div>';
          }
      ...
      async function submitManualConcern() {
        ...
        try {
          ...
          if(data.success) {
            ...
            logToConsole(\`Error event response: \${data.error}\`);
          }
        } catch (e) {
          logToConsole(\`Critical Network Transaction failure: \${e.message}\`);
        }
      }
    }
  </script>
